...
@Tags({"whq", "test"})
public class PutNifiTest extends AbstractProcessor {
    //外界参数说明
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            session.read(flowFile,(in)->{
                try(final InputStream input=new BufferedInputStream(in)){
                    System.out.println(tableName+"  "+new String(IOUtils.toByteArray(input), Charset.forName("utf8")));
                }
            });
